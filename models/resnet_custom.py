...
import sys
# sys.path.append('/home/ekansh.chauhan/HIPT/HIPT_4K/')
...
def resnet50_new(pretrained,key, **kwargs):
    model = ResNetTrunk(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        pretrained_url = get_pretrained_url_resnet(key)
        verbose = model.load_state_dict(torch.load(pretrained_url))
        print(verbose)
    return model

def uni_model(pretrained=False):
    ...
    if pretrained:
        pretrained_url = '/home/ekansh.chauhan/CLAM/model_weights/pytorch_model.bin'
        verbose = model.load_state_dict(torch.load(pretrained_url, map_location="cpu"), strict=True)
        print(verbose)
    return model
...
def get_pretrained_url_resnet(key):
    ...
    pretrained_url = "/home/ekansh.chauhan/CLAM/model_weights/" + model_zoo_registry.get(key)
    # pretrained_url = f"{URL_PREFIX}/{model_zoo_registry.get(key)}"
    return pretrained_url
